...
async (robin, mek, m, { from, body, isOwner }) => {
    const filePath = path.join(__dirname, '../oxzzy/autovoice.json');
...
async (robin, mek, m, { from, body, isOwner }) => {
    const filePath = path.join(__dirname, '../oxzzy/autosticker.json');
    const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    for (const text in data) {
        if (body.toLowerCase() === text.toLowerCase()) {
            const config = await readEnv();
...
async (robin, mek, m, { from, body, isOwner }) => {
    const filePath = path.join(__dirname, '../oxzzy/autoreply.json');
    const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    for (const text in data) {
        if (body.toLowerCase() === text.toLowerCase()) {
            const config = await readEnv();
